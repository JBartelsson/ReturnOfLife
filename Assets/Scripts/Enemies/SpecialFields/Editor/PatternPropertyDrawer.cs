...
public class PatternPropertyDrawer : PropertyDrawer
{
    ...
    {
        int gridSize = patternSO.Pattern.GridSize;
        if (gridSize <= 0) return;
        if (patternSO.Pattern.Data == null || patternSO.Pattern.Data.Length != (gridSize * gridSize) || reset)
        {
            patternSO.Pattern.Data = new Pattern.Field[gridSize, gridSize];
            for (int x = 0; x < gridSize; x++)
            {
                for (int y = 0; y < gridSize; y++)
                {
                    patternSO.Pattern.Data[x, y] =
                        new Pattern.Field(new Pattern.Index(x, y));
                }
            }

            if (!reset)
            {
                patternSO.Pattern.LoadDataString();
            }
            else
            {
